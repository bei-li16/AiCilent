...
- 供应商统计表固定显示 5 行，超出部分滚动条滑动，表头吸顶（此前条目多会撑长整个面板）
- "清零"按钮修复为真实清零：新增 `POST /api/stats/reset` 接口，清空所有供应商计数、总请求数、命中率曲线与最近尝试记录，并立即持久化（重启后不恢复）；此前仅在前端记基线显示差值，服务端数据未动
- 清零接口与启停开关同样受本机访问保护（需 `control_allow_remote` 才允许远程调用）
- 命中率曲线图例显示各优先级当前滚动成功率（如 `P1 27.4%`），命中耗时图例不变

**部署脚本完善**

- `deploy-rpi-interactive.ps1` 改为脚本相对路径定位，可在任意目录执行
- 配置文件改为必需：`config/providers.yaml` 不存在时直接报错提示（不再静默跳过），上传步骤固定执行
- 用户名/IP 输入自动去除首尾空白，避免 `Read-Host` 带空格导致连接失败
