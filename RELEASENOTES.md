...
- 实时日志新增「清空文件」按钮：截断服务端日志文件（`POST /api/logs/clear`，loopback 保护），历史备份 `proxy.log.时间戳` 保留；原「清空」按钮仅清空页面显示的行为不变
- Rotator 新增 `Clear()`（关闭→截断→重开），确保清空后继续追加写入
- 修复「最近上游尝试」折叠标题字号大于「供应商统计」的不一致（统一 16px）

## v1.0.22（2026-09-07）

**监控面板交互优化**

- 供应商统计排序改为：优先级升序 → 同优先级内最近活跃的排最前（新增 `last_activity` 字段记录每次尝试时间，清零时一并归零）
- 「供应商统计」与「最近上游尝试」改为可折叠区块，默认折叠，点击标题栏展开/收起（箭头指示状态），折叠时清零按钮仍可点
